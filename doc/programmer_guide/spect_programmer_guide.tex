...
\begin{itemize}
    ...
    \item 144 B read-only memory space in address range 0x4000 -- 0x408C.\newline
        (from ISA v0.2)
    \item 128 B write-only memory space in address range 0x5000 -- 0x507C.\newline
        (from ISA v0.2)
\end{itemize}
...
Fore more information about KBUS, see TROPIC01 Functional Specification, Section 16 \cite{TROPIC01}.

\TsSubSection{RBUS}

SPECT HW implements special 32 bit interface (RBUS) for requesting random numbers from the external systems RNG.
SPECT ISA provides possibility to fetch 32 bytes of random data with GRV instruction.

Fore more information about KBUS, see TROPIC01 Functional Specification, Section 17 \cite{TROPIC01}.
...
\begin{itemize}
    ...
    \item R31 = $P_{25519}$ for MUL25519 instruction.
    \item R31 = $P_{256}$ for MUL256 instruction.
    \item op2 < R31 and op3 < R31 for ADDP, SUBP instructions.
    \item R31 > 1 for ADDP, SUBP, MULP, REDP instructions.
\end{itemize}
...
\TropicNote{
    MUL25519 and MUL256 instruction offers much higher performance than MULP.
}
...
These memories are mapped in to SPECT memory space. Load and store instructions redirect read / write transactions to
SPECTs memory subsystem or to the EMEM interface depending on the address in Addr field of the instruction.
...
\TropicNote{
    SPECT compiler does not support expression parsing.
